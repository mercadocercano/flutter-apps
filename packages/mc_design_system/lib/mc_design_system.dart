/// Design System de Mercado Cercano.
///
/// Tokens, theme y widgets compartidos entre mc_pos y mc_consumer.
library;

export 'src/theme/mc_colors.dart';
export 'src/theme/mc_spacing.dart';
export 'src/theme/mc_typography.dart';
export 'src/theme/mc_theme.dart';

// Widgets
export 'src/widgets/mc_button.dart';
export 'src/widgets/mc_text_field.dart';
export 'src/widgets/mc_card.dart';

// Brand
export 'src/brand/mc_brand_palette.dart';
export 'src/brand/mc_brand_chip.dart';
